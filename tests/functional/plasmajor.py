...
diag_outputs = f"phare_outputs/test/plasmajor"
...
def get_taylor_spheromak(x, y, z, cx, cy, cz, R=1.0, B0=1.0):
    ...
    inside = r < R
    Br = np.where(inside, 2 * B0 * j1_over * ct, B_ext * ct * (1 - q))
    Bt = np.where(inside, -B0 * st * (j1_over + j1p), -B_ext * st * (1 + q / 2))
    Bp = np.where(inside, B0 * j1 * st, 0.0)
...
def plot():
    ...
    cmd = shlex.split(
        f"ffmpeg -r 10 -y -pattern_type glob -i '{plot_dir}/Bmag_t*.png' -c:v libx264 -crf 0 Bmag_plasmajor.mp4"
    )
