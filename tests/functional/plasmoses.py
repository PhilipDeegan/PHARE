...
def plot():
    ...
    for time in timestamps:
        run.GetB(time).plot(filename=plot_file_for_qty(plot_dir, f"Bx", time), qty=f"x")

    cmd = shlex.split(
        f"ffmpeg -r 10 -y -pattern_type glob -i '{plot_dir}/Bx_t*.png' -c:v libx264 -crf 0 Bx.mp4"
    )
