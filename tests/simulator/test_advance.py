    ...
    def base_test_overlaped_fields_are_equal(self, datahier, coarsest_time):
        ...
        for ilvl, overlaps in hierarchy_overlaps(datahier, coarsest_time).items():
            for overlap in overlaps:
                ...
                loc_b1 = amr_to_local(ovrlp_box, boxm.shift(pd1.ghost_box, offsets[0]))
                loc_b2 = amr_to_local(ovrlp_box, boxm.shift(pd2.ghost_box, offsets[1]))

                slice1 = boxm.select(pd1.dataset, loc_b1)
                slice2 = boxm.select(pd2.dataset, loc_b2)
