...
class ALayoutInitTest(InitializationTest):
    def compare_init(self, ndim, interp_order, qty, cmp_layout, atol=0, **kwargs):
        ...
        ref_hier = self.getHierarchy(
            ndim,
            interp_order,
            diag_outputs=f"init_{type(self)}_{_ref_layout}",
            **common,
        )
        cmp_hier = self.getHierarchy(
            ...
            diag_outputs=f"init_{type(self)}_{cmp_layout}",
            **common,
        )
...
class ALayoutAdvanceTest(AdvanceTestBase):
    def compare_advance(self, ndim, interp_order, qty, cmp_layout, atol=0, **kwargs):
        ...
        ref_hier = self.getHierarchy(
            ndim,
            interp_order,
            diag_outputs=f"adv_layout_{type(self)}_{_ref_layout}",
            **common,
        )
        cmp_hier = self.getHierarchy(
            ndim,
            interp_order,
            sim_setup_kwargs=dict(layout=cmp_layout),
            diag_outputs=f"adv_layout_{type(self)}_{cmp_layout}",
            **common,
        )
...
#             f"{self._testMethodName} ndim={ndim} interp={interp_order} layout={cmp_layout}"
#         )
#         self._compare_advance(ndim, interp_order, "eb", cmp_layout, atol=1e-3)
...
#             f"{self._testMethodName} ndim={ndim} interp={interp_order} layout={cmp_layout}"
#         )
#         self._compare_advance(ndim, interp_order, "moments", cmp_layout, atol=1e-5)
...
#             f"{self._testMethodName} ndim={ndim} interp={interp_order} layout={cmp_layout}"
#         )
#         self._compare_advance(ndim, interp_order, "particles", cmp_layout, atol=1e-5)
