...
struct InitFunctor
{
    ...
    Return operator()(Param x)
    {
        PHARE_LOG_LINE_SS(x.size());
...
struct Patch
{
        ...
        : layout{box}
    {
        ...
        // {
        //     PHARE_LOG_LINE_SS(*Bs[0].at(ConstArray<std::uint32_t, TestParam::dim>()));
        // }
        ...
        // if constexpr (TestParam::layout_mode == LayoutMode::AoSTS)
        // {
        //     PHARE_LOG_LINE_SS(em.E[0].at(ConstArray<std::uint32_t, TestParam::dim>()));
        //     PHARE_LOG_LINE_SS(*em.E[0].at(ConstArray<std::uint32_t, TestParam::dim>()));
        // }
    }
...
TYPED_TEST(GridFieldTest, test_compiles)
{
    ...
    for (auto& patch : this->patches)
    {
        if constexpr (TestParam::layout_mode == LayoutMode::AoSTS)
        {
            PHARE_LOG_LINE_SS(*patch.em.B[0].at(ConstArray<std::uint32_t, TestParam::dim>()));
            for (auto const& tile : patch.em.B[0])
            {
                PHARE_LOG_LINE_SS(*tile);
            }
        }

        PHARE_LOG_LINE_SS(patch.em.B[0].size());
