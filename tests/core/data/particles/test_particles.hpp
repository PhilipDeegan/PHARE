...
PHARE::core::Particle<dim> particle(std::array<int, dim> const& icell, std::size_t const id = 0)
{
    return {/*.weight = */ .001,
            /*.charge = */ .001,
            /*.iCell  = */ icell,
            /*.delta  = */ PHARE::core::ConstArray<double, dim>(.555),
            /*.v      = */ {{.505, .505, .505}},
            /*.v      = */ id};
}
                         ...
                         std::size_t const ghosts)
{
    ...
    // PHARE_LOG_LINE_STR(ppc << " " << magic_enum::enum_name(Particles::layout_mode) << " "
...
void add_particles_from(Src const& src, Dst& dst)
{
    ...
    if constexpr (Dst::layout_mode == LayoutMode::AoSPC)
