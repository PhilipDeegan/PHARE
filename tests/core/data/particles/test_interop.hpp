...
TYPED_TEST(ParticleListTest, SoAandAoSInterop)
{
    ...
    SoAParticleArray<dim> contiguous{size};
    ...
    for (auto const& it : contiguous)
    {
        auto i = particleArray.size();
        particleArray.emplace_back(std::copy(it));
        EXPECT_EQ(std::copy(contiguous.begin() + i), particleArray.back());
    }
    EXPECT_EQ(particleArray.size(), size);
    EXPECT_EQ(contiguous.size(), particleArray.size());

    contiguous.reserve(particleArray.capacity());
    EXPECT_EQ(contiguous.capacity(), particleArray.capacity());

    SoAParticleArray<dim> AoSFromSoA{particleArray.size()};
    ParticlePacker<dim>{particleArray}.pack(AoSFromSoA);

    std::size_t i = 0;
    for (auto const& it : AoSFromSoA)
        EXPECT_EQ(std::copy(it), particleArray[i++]);
}

TYPED_TEST(ParticleListTest, SoAArrayTypeConstruct)
{
    typename SoAParticleArray<1>::template array_type<2> refinedParticles;
}


TYPED_TEST(ParticleListTest, AoSArrayTypeConstruct)
{
    typename AoSMappedParticleArray<1>::template array_type<2> refinedParticles;
}