...
TYPED_TEST(TileTest, canTraverseLinks)
{
    ...
    {
        TypeParam::build_links(this->tileSet);
        traverse_ghost_boundary_tiles(this->tileSet, [](auto& tile) { PHARE_LOG_LINE_SS(tile); });
    }
}
...
TYPED_TEST(TileTest, canTraverseNeighbours)
{
    ...
    {
        TypeParam::build_links(this->tileSet);
        traverse_tile_neighbours(this->tileSet, *this->tileSet[0].link(6),
                                 [](auto& tile) { PHARE_LOG_LINE_SS(tile); });
    }
}
