...
template<std::size_t _dim, std::size_t _interp, auto lm, auto am, std::uint8_t _impl = 0>
struct TestParam
{
    ...
    using PhareTypes                  = PHARE::core::PHARE_Types<dim, interp, lm, am>;
...
auto patch_tile_field(TileSetTest_t& self)
{
    ...
    auto const ss0 = make_qty_nd_span_set_from<quanitity>(V[0], p0.layout);
...
PHARE_WITH_MKN_GPU(
    ,TestParam<3, 3,LayoutMode::AoSTS, AllocatorMode::GPU_UNIFIED, 2>
)
>;
// clang-format on
