                              ...
                              double const diff = 1e-15)
{
    ...
    log << "Fields compare (" << names << ") are == with ";
    log << "ok(" << eqvals << ") - ";
    log << "ok!=0(" << eqnot0 << ")  ";
...
// FieldTileSet<GridLayout_t, nd_array_t<double, A_>, HybridQuantity::Scalar>
template<bool binary_eq = false, typename GridLayout_, typename NdArray_>
EqualityReport
compare_fields(FieldTileSet<GridLayout_, NdArray_, HybridQuantity::Scalar> const& ref,
               FieldTileSet<GridLayout_, NdArray_, HybridQuantity::Scalar> const& cmp,
               [[maybe_unused]] double const diff = 1e-15)
{
    ...
    {
        auto const [eqvals, eqnot0, ref0, cmp0]
            = FloatFieldComparator_t{diff}(ref()[tidx], cmp()[tidx]);
