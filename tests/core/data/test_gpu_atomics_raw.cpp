...
template<auto opts, typename GridLayout_t>
auto make_ions(GridLayout_t const& layout)
{
    auto ions_p = std::make_shared<UsableIons_t<opts>>(layout, "protons");
    auto& ions  = *ions_p;
...
struct AtomicsTest : public ::testing::Test
{
    ...
    using Ions_t = UsableIons_t<opts>;

    GridLayout_t const layout{cells};
    // std::shared_ptr<Ions_t> ions = make_ions<opts>(*layout);
};
