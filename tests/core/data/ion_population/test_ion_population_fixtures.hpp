...
public:
    UsableIonsPopulation(initializer::PHAREDict const& dict, GridLayout_t const& layout)
        : Super{dict}
        , rho{this->name() + "_rho", layout, HybridQuantity::Scalar::rho, .1}
    ...
    UsableIonsPopulation(UsableIonsPopulation const&) = delete;
...
public:
    ...
    UsableIons(GridLayout_t const& layout, std::vector<std::string> const& pop_names)
        ...
        , rho{"rho", layout, HybridQuantity::Scalar::rho, .1}
        , Vi{"bulkVel", layout, HybridQuantity::Vector::V, .1}
    ...
    UsableIons(UsableIons const&) = delete;
