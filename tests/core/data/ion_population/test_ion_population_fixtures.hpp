...
struct UsableIonsDefaultTypes
{
    ...
    using Array_t             = NdArrayVector<dim>;
    using Field_t             = Field<Array_t, HybridQuantity::Scalar>;
    ...
    using IonPopulation_t = IonPopulation<ParticleArray_t, VecField_t, TensorField_t, GridLayout_t>;
};
...
class UsableIonsPopulation : public _defaults::IonPopulation_t
{
    ...
    using Super           = typename _defaults::IonPopulation_t;
...
public:
    UsableIonsPopulation(initializer::PHAREDict const& dict, GridLayout_t const& layout)
        ...
        , particles{make_particles<ParticleArray_t, GridLayout_t::nbrParticleGhosts()>(layout)}
    {
        F.set_on(std::get<0>(this->getCompileTimeResourcesUserList()));
        M.set_on(std::get<1>(this->getCompileTimeResourcesUserList()));
        this->setBuffer(this->name() + "_rho", &rho);
        this->setBuffer(this->name(), particles.pack());
    }
    ...
    typename _defaults::Field_t rho;
...
public:
    ...
    {
        Vi.set_on(std::get<0>(this->getCompileTimeResourcesUserList()));
        M.set_on(std::get<1>(this->getCompileTimeResourcesUserList()));
        this->setBuffer("rho", &rho);

        for (std::size_t i = 0; i < pop_names.size(); ++i)
            populations.emplace_back(pop_dict(pop_names[i]), layout);
        this->getRunTimeResourcesUserList().clear();
        for (auto& pop : populations)
            this->getRunTimeResourcesUserList().emplace_back(*pop);
    }
