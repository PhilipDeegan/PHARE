...
template<typename Ions, typename GridLayout_t>
auto& particle2mesh(Ions& ions, GridLayout_t const& layout)
{
    using Particles_t                = typename Ions::particle_array_type;
    auto constexpr static alloc_mode = Particles_t::alloc_mode;
    auto constexpr static dim        = GridLayout_t::dimension;
    ...
    using Interpolator_t
        = Interpolating<Particles_t, interp, alloc_mode == AllocatorMode::GPU_UNIFIED>;
    Interpolator_t{}.particleToMesh(*ions.populations[0].particles.domain_particles, layout,
                                    ions.populations[0].density(), ions.populations[0].flux());
    return ions;
}

template<typename Ions, typename GridLayout_t>
auto& boris_move(Ions& ions, GridLayout_t const& layout)
{
    using Particles_t                = typename Ions::particle_array_type;
    auto constexpr static alloc_mode = Particles_t::alloc_mode;
    auto constexpr static dim        = GridLayout_t::dimension;
    auto constexpr static interp     = GridLayout_t::interp_order;
    using EM                         = UsableElectromag<dim, alloc_mode>;
    using Interpolator_t
        = Interpolator<dim, interp, Particles_t::alloc_mode == AllocatorMode::GPU_UNIFIED>;

    EM em{layout};
    auto& particles  = ions.populations[0].particles.domain_particles;
    auto const& mass = ions.populations[0].mass();

    if constexpr (Particles_t::is_mapped)
    {
        using ParticleRange     = IndexRange<Particles_t>;
        using BoundaryCondition = PHARE::core::BoundaryCondition<dim, interp>;
        using Pusher_t = BorisPusher<dim, ParticleRange, typename EM::Super, Interpolator_t,
                                     BoundaryCondition, GridLayout_t>;
        auto noop      = [](auto& ps) { return ps; };
        auto range     = makeIndexRange(particles);
        Interpolator_t interpolator;
        Pusher_t{layout.meshSize(), dt}.move(range, range, *em, mass, interpolator, layout, noop,
                                             noop);
    }
    else
        SimpleBorisPusher<dim, Interpolator_t, GridLayout_t>{layout, layout.meshSize(), dt,
                                                             mass}(particles, *em);
...
void compare_particles(GridLayout_t const& layout, P0& ref, P1& cmp, std::size_t ghosts = 0)
{
...
struct IonUpdaterPPTest : public ::testing::Test
{
    ...
    // static inline DefaultIons<RefIons_t, dim, interp> ref;
...
// TYPED_TEST(IonUpdaterPPTest, particle2mesh)
// {
//     typename TestFixture::GridLayout_t layout{cells};
//     auto ref_ions = make_ions<typename TestFixture::RefParticleArray_t>(*layout);
//     auto cmp_ions = make_ions<typename TestFixture::CmpParticleArray_t>(*layout);

//     compare(*layout, particle2mesh(*ref_ions, *layout), particle2mesh(*cmp_ions, *layout));
// }

// TYPED_TEST(IonUpdaterPPTest, boris_move)
// {
//     typename TestFixture::GridLayout_t layout{cells};
//     auto ref_ions = make_ions<typename TestFixture::RefParticleArray_t>(*layout);
//     auto cmp_ions = make_ions<typename TestFixture::CmpParticleArray_t>(*layout);

//     // AoSPC removes ghosts during boris::move -- update to just select domain from each
//     if constexpr (TestFixture::CmpParticleArray_t::layout_mode != LayoutMode::AoSPC)
//         compare(*layout, boris_move(*ref_ions, *layout), boris_move(*cmp_ions, *layout));
// }
...
TYPED_TEST(IonUpdaterPPTest, updater)
{
    compare(*this->layout,   //
            *this->ref_ions, //
            evolve(*this->cmp_ions, *this->layout));
}
