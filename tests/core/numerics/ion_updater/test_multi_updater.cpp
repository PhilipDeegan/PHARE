...
auto static const seed      = get_env_as("PHARE_SEED", std::size_t{39});
...
void cmp_update(UpdaterMode mode, Patches& patches)
{
    ...
    boxings.try_emplace(
        "patch_id", Boxing_t{patches[0].layout,
                             grow(patches[0].layout.AMRBox(), GridLayout_t::nbrParticleGhosts())});
...
{
    using CPU_ref = ParticleArray<P0::dimension,
                                  ParticleArrayInternals<P0::dimension, LayoutMode::AoS,
...
struct MultiPatchIonUpdaterTest : public ::testing::Test
{
    ...
    void init_one()
    {
        ...
        {
            cmp_patches.emplace_back(layout, from_ions<CmpParticleArray_t>(layout, ref.ions));
            // cmp_patches.back().ions.populations[0].particles.domain_particles.sync();
        }
    }
    ...
    {
        bool static constexpr init_value_check = 0; // nonsense atm
...
using Permutations_t = testing::Types< // ! notice commas !
     ...
     // TestParam<3, LayoutMode::SoAVXTS, AllocatorMode::CPU,  UpdaterMode::all>
    // ,TestParam<3, LayoutMode::AoSMapped, AllocatorMode::CPU,  UpdaterMode::all>
    // ,TestParam<3, LayoutMode::SoA, AllocatorMode::CPU,  UpdaterMode::all>
...
PHARE_WITH_GPU(
    ...
    // ,TestParam<3, LayoutMode::SoATS, AllocatorMode::GPU_UNIFIED,  UpdaterMode::all>

    // ,TestParam<3, LayoutMode::AoSPC, AllocatorMode::GPU_UNIFIED,  UpdaterMode::domain_only>
    // ,TestParam<3, LayoutMode::SoAPC, AllocatorMode::GPU_UNIFIED,  UpdaterMode::domain_only>
    // ,TestParam<3, LayoutMode::AoSPC, AllocatorMode::GPU_UNIFIED,  UpdaterMode::all>
    // ,TestParam<3, LayoutMode::SoAPC, AllocatorMode::GPU_UNIFIED,  UpdaterMode::all>
