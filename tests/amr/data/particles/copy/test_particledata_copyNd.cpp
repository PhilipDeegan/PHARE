...
TYPED_TEST(CopyTest, PreservesAllParticleAttributesAfterCopy)
{
    ...
    auto const& destPatchGhostParticle = particle_fetcher(this->destData.patchGhostParticles);
    EXPECT_THAT(destPatchGhostParticle.iCell(), Eq(this->particle.iCell()));
    EXPECT_THAT(destPatchGhostParticle.delta(), Pointwise(DoubleEq(), this->particle.delta()));
    EXPECT_THAT(destPatchGhostParticle.v(), Pointwise(DoubleEq(), this->particle.v()));
    EXPECT_THAT(destPatchGhostParticle.weight(), DoubleEq(this->particle.weight()));
    EXPECT_THAT(destPatchGhostParticle.charge(), DoubleEq(this->particle.charge()));
}
...
{
    static constexpr auto dim = TypeParam::dim;
    auto const dimension      = SAMRAI::tbox::Dimension(this->dim);
