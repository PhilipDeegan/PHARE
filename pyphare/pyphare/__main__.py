...
class CliHelp:
    ...
    update = "Post op function for live data modifications"
...
def cli_args_parser():
    ...
    parser.add_argument("--update", default="", help=_help.update)
...
def find_update_in(cli_args, callable):
    ...
    for name, el in inspect.getmembers(module):
        if name==cli_args.update:
            print("update found! ", name)
            return el
    ...
    def _try_plot_axis(qty, k):
        try:
            ...
            hierarchy_from_sim(post_op.live, qty=qty).plot(
                ax=ax, qty=k, plot_patches=True, xlabel=qty, ylabel=""
            )
            _locals["idx"] += 1
