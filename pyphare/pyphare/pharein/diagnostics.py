...
def validate_timestamps(clazz, key, **kwargs):
    ...
    diff = np.abs(timestamps / sim.time_step - np.rint(timestamps / sim.time_step))
    # if not np.all(diff < 1e-9):
    #     print(timestamps / sim.time_step, np.rint(timestamps / sim.time_step))
    #     raise RuntimeError(
    #         f"Error: {clazz}.{key} is inconsistent with simulation.time_step: {diff}"
    #     )
