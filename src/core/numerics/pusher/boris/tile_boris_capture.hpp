...
public:
    ...
    static void move(MultiBoris<ModelAccessor>& in, auto const& boxings)
    {
        ...
        {
            MultiBoris<ModelAccessor>::domains.get(in.accessor.size());
            MultiBoris<ModelAccessor>::levelGhosts.get(in.accessor.size());
        }
    ...
    template<typename ModelAccessor>
    static void make_copy(MultiBoris<ModelAccessor>& in, auto const i)
    {
        auto view      = in.accessor[i];
        ...
        for (std::size_t j = 0; j < ions.size(); ++j)
        {
            ...
            MultiBoris<ModelAccessor>::domains[popidx]
                = std::make_shared<Particles_t>(pop.domainParticles());

            pop.levelGhostParticles().sync();
            MultiBoris<ModelAccessor>::levelGhosts[popidx]
    ...
    static void move_rest(MultiBoris<ModelAccessor>& in, auto const i)
    {
        ...
        for (std::size_t j = 0; j < ions.size(); ++j)
        {
            ...
            auto& domain
                = copy ? *MultiBoris<ModelAccessor>::domains[popidx] : pop.domainParticles();
            domain.reset_views();
            Functors<ParticleType::Domain, mode>{in, view, pop, domain, em}(in, i);

            auto& level_ghost = copy ? *MultiBoris<ModelAccessor>::levelGhosts[popidx]
