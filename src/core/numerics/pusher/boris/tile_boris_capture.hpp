...
template<typename MultiBorisPusher_t>
...
public:
    ...
    using Functors            = MultiBorisFunctors<This>;
    ...
    static void move(MultiBoris<ModelViews>& in)
    {
        ...
        // auto constexpr domain=[](auto& pop, auto const idx){
        //   if constexpr(copy)return; else return;
        // };
        ...
        in.streamer.host([&](auto const i) mutable {
            ...
            for (std::size_t j = 0; j < view.ions.size(); ++j)
            {
                ...
                Functors{in, view, pop,
                         copy ? MultiBoris<ModelViews>::domains[i + j] : pop.domainParticles()}();
                Functors{in, view, pop,
                         copy ? MultiBoris<ModelViews>::levelGhosts[i + j]
