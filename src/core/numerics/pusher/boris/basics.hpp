...
auto advance_noavx(Particles_t& ps, Args const&... args)
{
...
template<typename Particles_t, typename... Args>
void accelerate_noavx(Particles_t& ps, Args&... args)
{
...
template<typename Particles_t, typename... Args>
auto advance_avx(Particles_t& ps, Args const&... args)
{
    auto const& [pidx, halfDtOverDl] = std::forward_as_tuple(args...);
...
template<typename Particles_t, typename... Args>
void accelerate_avx(Particles_t& ps, Args&... args)
{
    auto const& [pidx, em, interp, layout, dto2m] = std::forward_as_tuple(args...);
    using Float                                   = std::decay_t<decltype(dto2m)>;
...
template<typename Particles_t, typename... Args>
auto avx_ad_acc_ad(Particles_t& ps, Args&... args)
{
