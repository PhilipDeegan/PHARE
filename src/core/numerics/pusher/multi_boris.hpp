...
struct MultiBoris
{
    ...
    using ParticleArray_v     = decltype(*std::declval<ParticleArray_t>());
        ...
        , particles{_particles(views)}
    {
        for (auto& view : views)
        {
            ...
            for (auto& pop : *view.ions)
            {
                ...
                each(1, pop.levelGhostParticles(), pop);
            }
        }
    }
...
public:
    template<typename ModelViews>
    static void move(MultiBoris<ModelViews>& in) _PHARE_ALL_FN_
    {
#if PHARE_HAVE_MKN_GPU
        ...
        auto per_particle = [=] _PHARE_ALL_FN_<bool accelerate = false>(auto const& i) mutable
        {
            ...
            auto const& newCell = advancePosition_<alloc_mode>(particle, halfDtOverDl[i]);
        ...
        streamer.async_dev([=] _PHARE_ALL_FN_(auto const i) mutable { per_particle(i); });
        streamer.host([ip = ip](auto const i) mutable {
            constexpr static std::uint32_t PHASE = 0;

            if (ip->particle_type[i] == 0) //  :(
                This::template sync<PHASE, ParticleType::Domain>(ip, i);
            else if (ip->particle_type[i] == 1)
                This::template sync<PHASE, ParticleType::Ghost>(ip, i);
            else
                throw std::runtime_error("No impl");
        });
        streamer.async_dev([=] _PHARE_ALL_FN_(auto const i) mutable {
            per_particle.template operator()<true>(i);
        });
        streamer.host([ip = ip](auto const i) mutable {
