...
public:
    ...
    void move(Particles& particles, Electromag const& em) const _PHARE_ALL_FN_
    {
        ...
        auto constexpr static layout_mode = Particles::layout_mode;
        ...
        if constexpr (layout_mode == LayoutMode::AoSPC)
        {
        ...
        {
            if constexpr (layout_mode == LayoutMode::SoAVX)
            {
                // advance accelerate advance
                boris::avx_ad_acc_ad(particles, em, interpolator_, layout_, halfDtOverDl_, dto2m);
            }
            else
                   ...
                   std::array<double, dim> const halfDtOverDl, double const& dto2m) _PHARE_ALL_FN_
    {
        ...
        particle.iCell() = boris::advance<alloc>(particle, halfDtOverDl);
            ...
            for (auto const& bix : particles.local_box())
                for (std::size_t idx = 0; idx < particles.size(bix); ++idx)
                {
                    ...
                    if constexpr (accelerate)
                        boris::accelerate(particle, interpolator_.m2p(particle, em, layout_),
                                          dto2m);
    ...
    void move_aos_pc_gpu_impl(Particles& particles, Electromag const& em) const
    {
        PHARE_WITH_MKN_GPU({
            ...
            auto per_particle = [=] _PHARE_ALL_FN_<bool accelerate = false>() mutable
            {
                ...
                auto& particle = *it;
                if constexpr (accelerate)
                    boris::accelerate(particle, interp.m2p(particle, em, layout), dto2m_);
