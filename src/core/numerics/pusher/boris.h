...
namespace core
{
    ...
    public:
                              ...
                              GridLayout const& layout) override
        {
            ...
            // stop at newEnd.
            auto eb = interpolator(rangeOut.begin(), rangeOut.end(), emFields, layout);

            // get the particle velocity from t=n to t=n+1
            accelerate_(rangeOut, rangeOut, mass, eb);
    ...
    private:
         ...
         */
        template<typename ParticleRange, typename ParticleElectromag>
        void accelerate_(ParticleRange inputParticles, ParticleRange outputParticles, double mass,
                         ParticleElectromag const& particleElectromag)
        {
            ...
            {
                auto const& [Ex, Ey, Ez] = particleElectromag[i][0];
                auto const& [Bx, By, Bz] = particleElectromag[i][1];
