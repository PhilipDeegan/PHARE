...
public:
                         ...
                         double coef = 1.) _PHARE_DEV_FN_
    {
        ...
        auto fy   = make_array_view(&data[ziz * 4], flux[1].shape());
        auto fz   = make_array_view(&data[ziz * 5], flux[2].shape());
        ...
        auto& parts = tile();
        {
            ...
            __syncthreads();
            for (; pid < each; ++pid)
                interp.particleToMesh(parts[(pid * ws) + tidx], pdensity, cdensity, flux,
                                      /*layout,*/ tbox, coef);
        ...
        auto const each = rhop.size() / ws;
        pid             = 0;
        for (; pid < each; ++pid)
        {
            auto const i = (pid * ws) + tidx;
            pdensity.data()[i] += rhop.data()[i];
            pdensity.data()[i] += rhoc.data()[i];
