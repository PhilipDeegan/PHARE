...
namespace core
{
    ...
    int computeStartLeftShift([[maybe_unused]] double delta)
    {
        ...
        void indexAndWeights_(GridLayout const& layout, Particle const& part)
        {
            for (auto iDim = 0u; iDim < dimension; ++iDim)
            {
                ...
                startIndex_[centering2int(centering)][iDim]
                    = iCell[iDim]
                      - computeStartLeftShift<CenteringT, centering, interpOrder>(part.delta[iDim]);

                weightComputer_.computeWeight(iCell[iDim] + part.delta[iDim],
                                              startIndex_[centering2int(centering)][iDim],
