                                          ...
                                          auto const& boxing, double const dt)
{
    ...
    if (mode == UpdaterMode::copy)
        update_copy(ions, em, boxing, dt);

    else
        update_ref(ions, em, boxing, dt);
}



/**
 * @brief IonUpdater0<Ions>::update_copy
   evolves moments from time n to n+1 without updating particles, which stay at time n
 */
template<typename Ions>
void IonUpdater0<Ions>::update_copy( //
    auto& ions, auto const& em, auto const& boxing, double const dt)
{
    PHARE_LOG_SCOPE(3, "IonUpdater0::update_copy");
 ...
 */
template<typename Ions>
void IonUpdater0<Ions>::update_ref( //
    auto& ions, auto const& em, auto const& boxing, double const dt)
{
    PHARE_LOG_SCOPE(3, "IonUpdater0::update_ref");
