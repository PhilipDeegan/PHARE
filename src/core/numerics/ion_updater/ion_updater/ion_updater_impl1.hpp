...
public:
    ...
    void updatePopulations(UpdaterMode const mode, auto& ions, auto&&... args) const;

    auto static make_pusher(auto const& layout, auto const& dt)
    {
        auto const ms = layout.meshSize();
        return Pusher{dt, for_N_make_array<dimension>([&](auto i) { return 0.5 * dt / ms[i]; })};
    }

    class Pusher;

private:
    void updateAndDepositDomain_(auto& ions, auto const& em, auto const& boxing,
                                 double const dt) const;

    void updateAndDepositAll_(auto& ions, auto const& em, auto const& boxing,
                              double const dt) const;
...
template<typename Ions>
void IonUpdater1<Ions>::updatePopulations(UpdaterMode const mode, auto& ions, auto&&... args) const
{
                                                ...
                                                double const dt) const
{
    ...
    for (auto& pop : ions)
        Pusher{boxing.layout, dt, pop.mass()}.template //
...
void IonUpdater1<Ions>::updateAndDepositAll_( //
    auto& ions, auto const& em, auto const& boxing, double const dt) const
{
