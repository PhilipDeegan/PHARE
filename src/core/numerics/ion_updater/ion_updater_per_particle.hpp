...
public:
    ...
    auto static constexpr interp_order = GridLayout::options.interp_order;
...
{
    return ParticleArrayPartitioner<Particles::alloc_mode, Particles>{particles}(std::array{
        layout.AMRBox(), grow(layout.AMRBox(), GridLayout::options.particle_ghost_width)});
}
