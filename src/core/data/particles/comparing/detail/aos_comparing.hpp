                                                                       ...
                                                                       PS1 const& ps1)
{
    ...
    for (std::size_t i = 0; i < ps0.size(); ++i)
        if (auto eq = comparator(ps0()[i], ps1()[i]); !eq)
            return eq;
...
template<typename PS0, typename PS1>
EqualityReport
ParticlesComparator<AoSTS, GPU_UNIFIED, AoSTS, GPU_UNIFIED>::operator()(PS0 const& ps0,
                                                                        PS1 const& ps1)
{
    return ParticlesComparator<AoSTS, CPU, AoSTS, CPU>{}(ps0, ps1); // do better
}
