...
auto constexpr chunk_type_helper()
{
    ...
    else if constexpr (o.layout_mode == AoSPCTS)
    {
        ...
        using CellIt
            = decltype(std::declval<PerTile&>().local_box(std::declval<PerTile&>().box()).begin());
...
// and, for each one, walks that container's own domain cells -- flattening
// both levels of nesting down to the leaf particle chunks, with no allocation.
...
class PerCellTileChunkSpan
{
    ...
    using CellBox = decltype(std::declval<PerTileContainer_t&>().local_box(
        std::declval<PerTileContainer_t&>().box()));
...
public:
    ...
    private:
        // advance outer_it_ to the next tile with at least one domain cell,
        // positioning cell_it_/cell_end_ on it; stops at outer_end_ if none left
        void enter_tile()
        {
            for (; outer_it_ != outer_end_; ++outer_it_)
            {
                auto& pc  = *outer_it_;
                cell_box_ = pc.local_box(pc.box());
