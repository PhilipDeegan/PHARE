...
void move_in_domain(Dst& dst, Src& src, Box_t const& domain_box)
{
    ...
    static_assert(Dst::layout_mode == LayoutMode::AoSTS);
    static_assert(Src::layout_mode == LayoutMode::AoSTS);
...
template<typename Dst, typename Src, typename Box_t> //
void move_in_ghost_layer(Dst& dst, Src& src, Box_t const& domain_box, Box_t const& ghost_box)
{
    // todo move to dispatcher and optimize
    static_assert(Dst::layout_mode == LayoutMode::AoSTS);
    static_assert(Src::layout_mode == LayoutMode::AoSTS);
