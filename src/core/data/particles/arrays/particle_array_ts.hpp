...
public:
    ...
    ParticlesTile(This&&)      = default;
    ParticlesTile(This const&) = default;

    template<typename Ps, typename Nd, auto S0 = Particles::storage_mode,
             auto S1  = Ps::storage_mode,
             typename = std::enable_if_t<S0 == StorageMode::SPAN and S1 != S0>>
...
public:
    ...
    // TileSetVector(TileSetVector const& from)            = delete;
    // TileSetVector(TileSetVector&& from)                 = default;
    // TileSetVector& operator=(TileSetVector&& from)      = default;
    // TileSetVector& operator=(TileSetVector const& from) = delete;
    ...
    template<typename GridLayout_t>
    auto& insert_domain_from(TileSetVector& src, GridLayout_t const& layout);
                                                         ...
                                                         GridLayout_t const& /*layout*/)
{
    ...
    auto const per_tile = [&](auto& src_tile) {
        ...
        auto const per_neighbour = [&](auto const& neigh) {
            if (remaining.size() == 0)
                return;
            ...
            if (auto const size = end - start)
            {
                ...
                append_particles<ParticleType::Domain>(
                    src_tile().view(start, size),
                    dst_tile() /*,
layout*/);
            }
