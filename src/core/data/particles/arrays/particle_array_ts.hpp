...
public:
    ...
    {
        return as_local_cell(box_, icell);
    }
...
public:
    ...
    auto local_cell(std::array<int, dim> const& icell) const { return as_local_cell(box_, icell); }
    auto local_cell(Point<int, dim> const& icell) const { return local_cell(icell.toArray()); }
    auto local_box() const
    {
        return box_from_zero_to_upper_minus_one(box_.shape().template toArray<std::uint32_t>());
    }
...
struct TileSetParticles : public Super_
{
    ...
    {
        auto const& tile = *particles_.at((Point{arr} - Super::safe_box().lower).as_unsigned());
