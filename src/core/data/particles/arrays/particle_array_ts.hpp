...
public:
    ...
    {
        if (!particles_.at(local_cell(cell)))
        {
            PHARE_LOG_LINE_SS(Point{cell});
        }
...
void TileSetVector<Particles, impl>::sync_check_realloc()
{
    ...
    PHARE_LOG_SCOPE(3, "TileSetVector::sync_gpu_gaps_and_tmp::reserve_scan ");
...
struct TileSetParticles : public Super_
{
    ...
    Super& operator*()
    {
        Super::template sync<2>();
        check();
        for (auto const& tile : *this)
            for (auto const& particle : tile())
            {
                assert(particle.iCell()[0] < 1000);
            }
