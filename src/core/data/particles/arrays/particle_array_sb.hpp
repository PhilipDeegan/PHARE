...
public:
    ...
    using This                       = SubBoxSpan<Particles, impl_v>;
    using lobox_t                    = Box<std::uint32_t, dim>;

private:
    using locell_t = std::array<std::uint32_t, dim>;

    template<typename SubBoxArray>
    auto resolve(SubBoxArray& arr)
    {
        if constexpr (SubBoxArray::storage_mode == StorageMode::SPAN)
    ...
    template<typename SubBoxArray>
    auto resolve_gaps(SubBoxArray& arr)
    {
        if constexpr (SubBoxArray::storage_mode == StorageMode::SPAN)
            return arr.gaps_;
        else
...
{
    auto static constexpr dim = Particles::dimension;
    using This                = SubBoxVector<Particles, impl_>;

    template<typename P, std::uint8_t i>
    friend class SubBoxSpan;
...
public:
    ...
    SubBoxVector(box_t const& box = {}, std::size_t ghost_cells = 0)
    ...
    SubBoxVector(SubBoxVector const& from)            = default;
    SubBoxVector(SubBoxVector&& from)                 = default;
    SubBoxVector& operator=(SubBoxVector&& from)      = default;
    SubBoxVector& operator=(SubBoxVector const& from) = default;
    ...
    auto& insert(SubBoxVector const& src);

    auto& insert_domain_from(SubBoxVector const& src);
    ...
    void cap()
    {
        PHARE_LOG_SCOPE(1, "SubBoxVector::cap");
...
}; // SubBoxVector<Particles>



template<typename Particles, std::uint8_t impl>
auto& SubBoxVector<Particles, impl>::insert(SubBoxVector const& src)
{
    std::size_t added = 0;
...
template<typename Particles, std::uint8_t impl>
auto& SubBoxVector<Particles, impl>::insert_domain_from(SubBoxVector const& src)
{
...
template<typename Particles, std::uint8_t impl>
template<auto type>
auto& SubBoxVector<Particles, impl>::reserve_ppc(std::size_t const& ppc)
{
...
template<typename Particles, std::uint8_t impl>
template<auto type>
void SubBoxVector<Particles, impl>::trim()
{
    static_assert(std::is_same_v<decltype(type), ParticleType>);
...
template<typename Particles, std::uint8_t impl>
template<auto type>
void SubBoxVector<Particles, impl>::sync_cpu_gaps_and_tmp()
{
...
template<typename Particles, std::uint8_t impl>
template<auto type>
void SubBoxVector<Particles, impl>::sync_gpu_gaps_and_tmp_impl0()
{
    auto const lbox = local_box();

    {
        PHARE_LOG_SCOPE(1, "SubBoxVector::sync_gpu_gaps_and_tmp::reserve_scan ");
        for (auto const& bix : lbox)
    ...
    {
        PHARE_LOG_SCOPE(1, "SubBoxVector::sync_gpu_gaps_and_tmp::add ");
        for (auto const& bix : lbox)
    ...
    {
        PHARE_LOG_SCOPE(1, "SubBoxVector::sync_gpu_gaps_and_tmp::delete ");
        for (auto const& bix : lbox)
...
template<typename Particles, std::uint8_t impl>
template<auto type>
void SubBoxVector<Particles, impl>::sync_gpu_gaps_and_tmp_impl1()
{
}
...
template<typename Particles, std::uint8_t impl>
template<auto type>
void SubBoxVector<Particles, impl>::sync_gpu_gaps_and_tmp()
{
    // PHARE_LOG_LINE_STR("sync_gpu_gaps_and_tmp " << magic_enum::enum_name(type));
    PHARE_LOG_SCOPE(1, "SubBoxVector::sync_gpu_gaps_and_tmp ");
...
template<typename Particles, std::uint8_t impl>
void SubBoxVector<Particles, impl>::reset_index_wrapper_map()
{
...
void SubBoxVector<Particles, impl>::sync()
{
    static_assert(std::is_same_v<decltype(type), ParticleType>);
    static_assert(type != ParticleType::All);

    PHARE_LOG_SCOPE(1, "SubBoxVector::sync");
    ...
    {
        PHARE_LOG_SCOPE(1, "SubBoxVector::sync::reset");
    ...
    {
        PHARE_LOG_SCOPE(1, "SubBoxVector::sync::reset_views");
        reset_views();
    }
}
...
struct SubBoxParticles : public Super_
{
    using Super      = Super_;
    using This       = SubBoxParticles<Super>;
    ...
    SubBoxParticles(SubBoxParticles const& from)            = default;
    SubBoxParticles(SubBoxParticles&& from)                 = default;
    SubBoxParticles& operator=(SubBoxParticles&& from)      = default;
    SubBoxParticles& operator=(SubBoxParticles const& from) = default;
...
template<typename T>
struct SubBoxParticles<OuterSuper>::iterator_impl
{
...
template<typename Super_>
template<typename T>
struct SubBoxParticles<Super_>::index_wrapper
{
...
template<typename Particles, std::uint8_t impl>
template<std::uint8_t PHASE, auto type>
void SubBoxSpan<Particles, impl>::sync() _PHARE_ALL_FN_
{
    PHARE_LOG_SCOPE(1, "SubBoxSpan::sync()");
...
void SubBoxSpan<Particles, impl>::sync(Args&&... args) _PHARE_ALL_FN_
{
    PHARE_LOG_SCOPE(1, "SubBoxSpan::sync(stream)");
...
template<typename Particles, std::uint8_t impl>
template<std::uint8_t PHASE, auto type>
void SubBoxSpan<Particles, impl>::sync_add_new() _PHARE_ALL_FN_
{
...
template<typename Particles, std::uint8_t impl>
template<std::uint8_t PHASE, auto type>
void SubBoxSpan<Particles, impl>::sync_rm_left() _PHARE_ALL_FN_
{
#if PHARE_HAVE_MKN_GPU
    auto const& kidx = mkn::gpu::idx();
    auto const& bix  = *(local_ghost_box_.begin() + kidx);
