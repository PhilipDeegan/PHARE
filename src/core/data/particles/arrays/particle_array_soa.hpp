...
public:
    ...
    {
        auto const& [w, c, i, d, v] = particle;
        Super::get_vec(this->weight_).emplace_back(w);
        Super::get_vec(this->charge_).emplace_back(c);
        Super::get_vec(this->iCell_).emplace_back(i);
        Super::get_vec(this->delta_).emplace_back(d);
        Super::get_vec(this->v_).emplace_back(v);
        return end() - 1;
    }
    ...
    template<auto S = storage_mode, typename = std::enable_if_t<S == StorageMode::VECTOR>>
    void emplace_back(This const& src, std::size_t const& idx)
    {
        auto this_tuple = as_tuple();
        auto that_tuple = src.as_tuple();
        for_N<std::tuple_size_v<decltype(this_tuple)>>([&](auto vi) {
