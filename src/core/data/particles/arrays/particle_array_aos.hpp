...
template<std::size_t dim> // todo const template param?
class AoSSpan
{
public:
    ...
    // using container_type            = Span<Particle_t>;
    // using iterator                  = Particle_t*;
    // using const_iterator            = Particle_t const*;
    ...
    // auto begin() const _PHARE_ALL_FN_ { return particles_.begin(); }
    // auto begin() _PHARE_ALL_FN_ { return particles_.begin(); }
    // auto end() const _PHARE_ALL_FN_ { return particles_.end(); }
    // auto end() { return particles_.end(); }
...
public:
    ...
    // auto static constexpr is_host_mem = alloc_mode == AllocatorMode::CPU;
    using Particle_t     = Particle<dim>;
    using value_type     = Particle_t;
    using vec_helper     = PHARE::Vector<Particle_t, alloc_mode>;
...
struct AoSParticles : public Super_
{
    ...
    auto static constexpr storage_mode = Super_::storage_mode;
