...
public:
    ...
    using Vector = typename Consts::stack_var;
    ...
    explicit NdArrayVector(std::array<std::uint32_t, dim> const& ncells)
        : Vector{vec_helper::make(core::product(ncells))}
        , View{Vector::var.data(), ncells}
    {
    }


    NdArrayVector(NdArrayVector const& that)
        : Vector{vec_helper::from(that.var)}
        , View{Vector::var.data(), that.shape()}
    {
    }

    template<std::size_t d, typename T, bool c, auto a, typename C>
    NdArrayVector(NdArrayVector<d, T, c, a, C>&& that)
        : Vector{vec_helper::from(std::move(that.var))}
        , View{Vector::var.data(), that.shape()}
    {
    }
    ...
    {
        Vector::var = std::move(vec_helper::make(core::product(ncells)));
