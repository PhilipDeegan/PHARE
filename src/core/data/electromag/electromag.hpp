...
template<typename Electromag_t>
void check_electromag(Electromag_t const& em)
{
    auto check = [](auto const& vf) {
        for (auto const& c : vf)
            for (auto const& tile : c())
                for (auto const& v : tile())
                    if (v == 0)
                        throw std::runtime_error("bad electromag");
    };
