...
public:
    ...
    template<typename Operator, typename A>
    void set_from(std::vector<value_type, A> const& vec, std::size_t seek = 0);

    template<typename A>
    void append_to(std::vector<value_type, A>& vec);
};
...
template<typename Operator, typename A>
void FieldBox<Field_t>::set_from(std::vector<value_type, A> const& vec, std::size_t seek)
{
...
template<typename Field_t>
template<typename A>
void FieldBox<Field_t>::append_to(std::vector<value_type, A>& vec)
{
