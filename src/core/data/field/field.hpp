...
public:
    ...
    // template<typename S = NdArray, std::enable_if_t<!std::is_same_v<S, data_view_t>>>
             ...
             Field<NdArrayImpl, PhysicalQuantity>& avg)
{
    ...
    if constexpr (CompileOptions::WithUmpire && CompileOptions::WithRAJA)
    {
        auto d_f1  = f1.data();
        auto d_f2  = f2.data();
        auto d_avg = avg.data();
        ...
        for (std::size_t i = 0; i < f1.size(); ++i)
            d_avg[i] = (d_f1[i] + d_f2[i]) * .5; // also bad

        // probably change to
        // PHARE_WITH_RAJA(PHARE::raja::exec(
        //     [=] RAJA_DEVICE(int i) mutable { d_avg[i] = (d_f1[i] + d_f2[i]) * .5; }, f1.size()));
    }
    else
    {
