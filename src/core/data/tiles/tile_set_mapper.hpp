...
struct ATiler
{
    ...
    Box_t const& box;
    std::array<std::uint32_t, dimension> const& shape;
    std::array<std::size_t, dimension> const& tile_size;
};
...
{
    tile_set() = Tiler<TileSet_t, impl>{{tile_set.box(), tile_set.shape(), tile_set.tile_size()}}.f(
        args...);
}
