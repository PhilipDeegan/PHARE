...
auto Tiler<TileSet_t, 3>::f(Args&&... args)
{
    ...
    auto const& shape                  = this->box.shape();
    auto& tiles                        = this->tile_set();
    std::size_t const min_before_split = 8; // 8;
    ...
    auto split_1d = [](int const length) {
        ...
        if (length == 9)
            return std::vector<int>{5, 4};
        if (length == 10)
            return std::vector<int>{5, 5};
        if (length < 13)
            return std::vector<int>{6, length - 6};
    ...
    // {
    //     std::cout << __FILE__ << " " << __LINE__ << " " << tile << std::endl;
    // }
}
