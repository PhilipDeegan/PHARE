...
public:
    ...
    /**
     * @brief Parallelize a loop by splitting it into blocks, submitting each block separately to
     * the thread pool, and waiting for all blocks to finish executing. The user supplies a loop
     * function, which will be called once per block and should iterate over the block's range.
     *
     ...
     * from first_index to (index_after_last - 1) inclusive. In other words, it will be equivalent
     * to "for (T i = first_index; i < index_after_last; i++)". Note that if first_index ==
     * index_after_last, the function will terminate without doing anything.
     ...
     * loop(start, end) should typically involve a loop of the form "for (T i = start; i < end;
     * i++)".
     * @param num_blocks The maximum number of blocks to split the loop into. The default is to use
     * the number of threads in the pool.
     */
    template<typename T1, typename T2, typename F>
    void parallelize_loop(const T1& first_index, const T2& index_after_last, const F& loop,
                          ui32 num_blocks = 0)
    {
     ...
     * number of threads. Any tasks that were waiting in the queue before the pool was reset will
     * then be executed by the new threads. If the pool was paused before resetting it, the new pool
