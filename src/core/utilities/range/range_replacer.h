#ifndef PHARE_CORE_UTILITITES_RANGE_REPLACER_H
#define PHARE_CORE_UTILITITES_RANGE_REPLACER_H
...
struct RangeReplacer
{
    ...
    void replace(Container& src, Range_t src_range, bool copy_src = false, bool erase_src = false)
    {
        ...
        if (src_size > 0)
        {
            // if (threads_ == 1) // invalides other ranges, if there is only one it might be ok
            //     std::copy(src_begin, src_range.end(), std::back_inserter(dst));
            // else
        ...
        if (erase_src)
        {
            ...
            // if (threads_ == 1) // invalides other ranges
            //     src.erase(src.begin() + offset, src.begin() + offset + src_range.size());
            // else
...
#endif // PHARE_CORE_UTILITITES_RANGE_REPLACER_H
