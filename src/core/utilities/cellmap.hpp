...
public:
    ...
    void swap(cell_t const a, cell_t const b, std::size_t const i0, std::size_t const i1);
...
void CellMap<dim, cell_index_t>::swap(cell_t const a, cell_t const b, std::size_t const i0,
                                      std::size_t const i1)
{
    auto& indexesA = cellIndexes_(local_(a));
    auto& indexesB = cellIndexes_(local_(b));

    assert(indexesA.is_indexed(i0));
    assert(indexesB.is_indexed(i1));

    // find before modify!
    auto const posA = indexesA.position(i0);
    auto const posB = indexesB.position(i1);

    indexesA.update(posA, i1);
    indexesB.update(posB, i0);
}
