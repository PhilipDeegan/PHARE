...
{
public:
    template<core::ParticleType ptype = core::ParticleType::Domain, typename H5File,
             typename Particles>
    static void write(H5File& h5file, Particles const& particles, std::string const& path)
    {
        ...
        else
        {
            ...
            core::SoAParticleArray<dim> copy{Packer::template size<ptype>(particles)};
            packer.template pack<ptype>(copy);
            write_(copy);
        }
    }

    template<std::size_t dim, core::ParticleType ptype = core::ParticleType::Domain, typename T,
             typename Particles>
    NO_DISCARD auto static size_for(T const& type, Particles const& particles)
    {
        auto const n_particles = core::ParticlePacker<Particles>::template size<ptype>(particles);
        if (n_particles == 0)
