    ...
    int const ilvl, auto const& amr_slice_box)
{
    ...
    auto const& rank_boxes = hier_data.level_boxes_per_rank[ilvl];
    std::vector<int> rank_data_sizes(mpi::size(), 0);
    ...
    int const ilvl, auto const& amr_slice_box)
{
    ...
    std::vector<int> n_isect_per_rank(mpi::size(), 0);
    for (int r = 0; r < mpi::size(); ++r)
...

    {
        ...
        append_step_offset(step_level + lvl + "/NumberOfAMRBox", total_isect);
    }
    ...
    for (auto const& box : rank_boxes[mpi::rank()])
