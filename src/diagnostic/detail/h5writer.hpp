                                   ...
                                   double timestamp)
{
    ...
    // per-model attributes (dimension/interpOrder/layoutType/origin) are left out here as they
    // aren't uniform across models sharing this writer; these are hierarchy-level, not model
...
void H5Writer<ModelMapper_t>::writeDatasets_(std::vector<DiagnosticProperties*> const& diagnostics)
{
    ...
    std::size_t maxLocalLevel = 0;
    // getPatchProperties is purely a function of gridLayout - collected once, independent
    // of which diagnostics are active
