    ...
    namespace h5
    {
                                                                      ...
                                                                      Attributes& patchAttributes)
        {
            ...
            auto particleInfo = [&](auto& attributes, auto& particles) {
                size_t part_idx = 0;
                packer          = std::make_unique<ParticlePacker<dimension>>(particles);
                std::apply(
                    [&](auto&... args) {
                        (
                            [&]() {
            ...
            for (auto& pop : outer.modelView().getIons())
            {
                ...
                particleInfo(patchAttributes[patchID][popIdx]["levelGhost"],
                             pop.levelGhostParticles());
                particleInfo(patchAttributes[patchID][popIdx]["patchGhost"],
