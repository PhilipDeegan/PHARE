...
public:
    ...
    using Packer    = core::ParticlePacker<typename H5Writer::Model_t::particle_array_type>;
...
{
    auto checkInfo = [&]<core::ParticleType ptype>(auto& tree, auto pType, auto& attr, auto& ps) {
        std::string const active{tree + pType};
        if (diagnostic.quantity == active)
        {
            std::size_t part_idx = 0;
            core::apply(Packer::empty(), [&](auto const& arg) {
                attr[pType][Packer::keys()[part_idx++]]
                    = hdf5::ParticleWriter::size_for<dimension, ptype>(arg, ps);
            });
        }
    };
    ...
    for (auto& pop : h5Writer.modelView().getIons())
    {
        ...
        checkInfo.template operator()<core::ParticleType::Domain>(tree, "domain", popAttr,
                                                                   pop.domainParticles());
        checkInfo.template operator()<core::ParticleType::LevelGhost>(
            tree, "levelGhost", popAttr, pop.levelGhostParticles());
    }
}
...
void ParticlesDiagnosticWriter<H5Writer>::write(DiagnosticProperties& diagnostic)
{
    ...
    auto checkWrite = [&]<core::ParticleType ptype>(auto& tree, auto pType, auto& ps) {
        std::string active{tree + pType};
        if (diagnostic.quantity == active && ps.size() > 0)
            hdf5::ParticleWriter::write<ptype>(this->h5FileForQuantity(diagnostic), ps,
                                               h5Writer.patchPath() + "/");
    };
    ...
    for (auto& pop : h5Writer.modelView().getIons())
    {
        ...
        checkWrite.template operator()<core::ParticleType::Domain>(tree, "domain",
                                                                    pop.domainParticles());
        checkWrite.template operator()<core::ParticleType::LevelGhost>(
            tree, "levelGhost", pop.levelGhostParticles());
    }
}
