...
private:
        ...
        if constexpr (solver::is_hybrid_model_v<Model_t>)
        {
            ...
            accessors.dispatch(
                ...
                [&](auto& field, std::string const&, std::string const&) {
                    using Quantity = std::decay_t<decltype(field)>;
                    if constexpr (std::is_same_v<Quantity, typename Accessors::Field_t>)
                        file_writer.writeField(modelView, field, layout);
                    else
                        file_writer.template writeTensorField<Quantity::rank>(modelView, field,
                                                                              layout);
                });
        }
    }
            ...
            accessors.dispatch(
                ...
                [&](auto& field, std::string const&, std::string const&) {
                    ...
                    if constexpr (std::is_same_v<Quantity, typename Accessors::Field_t>)
                        ...
                        file_writer.template writeTensorField<1>(modelView, field, layout);
                });
        }
    }
    ...
    void computeMhdFluid(ModelView& modelView, DiagnosticProperties& diagnostic)
    {
        ...
        if constexpr (solver::is_mhd_model_v<Model_t>)
        {
            ...
            auto const gamma
                = diagnostic.fileAttributes["heat_capacity_ratio"]
                      .template to<double>(); // or FloatType if we want to expose that to
