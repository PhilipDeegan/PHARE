    ...
    public:
                                ...
                                bool isRegridding) override
        {
            ...
            for (auto& patch : level)
            {
                ...
                {
                    PHARE_LOG_LINE_SS(core::sum_field(pop.density()));
                    PHARE_LOG_LINE_SS(core::sum_field(pop.flux()[0]));
                }
            }
            ...
            for (auto& patch : level)
            {
                ...
                for (auto const& pop : ions)
                {
                    ...
                    PHARE_LOG_LINE_SS(core::sum_field(reduce_single(pop.flux()[0])));
                }

                ions.computeDensity();
                ions.computeBulkVelocity();

                PHARE_LOG_LINE_SS(core::sum_field(ions.density()));
            }
