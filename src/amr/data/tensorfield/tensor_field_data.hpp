...
class TensorFieldData : public SAMRAI::hier::PatchData
{
    ...
    auto dual_dir_minus_1(auto box, auto qty) const
    {
        auto const centerings = GridLayoutT::centering(qty);
        for (std::uint16_t i = 0; i < dimension; ++i)
            box.upper[i] -= centerings[i] == core::QtyCentering::dual ? 1 : 0;
        return box;
    }
        ...
        if (transformation.getRotation() == SAMRAI::hier::Transformation::NO_ROTATE)
        {
            for (auto const& box : fieldOverlap.getDestinationBoxContainer())
            {
                for (std::size_t c = 0; c < N; ++c)
                {
                    ...
                    core::FieldBox<Grid_t const> src{
                        source, gridLayout,
                        dual_dir_minus_1(phare_box_from<dimension>(packBox),
                      ...
                      auto& dst_grids)
    {
        ...
        for (std::size_t c = 0; c < N; ++c)
            for (auto const& sambox : fieldOverlap.getDestinationBoxContainer())
            {
                ...
                auto const box = dual_dir_minus_1(phare_box_from<dimension>(sambox),
...
private:
               ...
               TensorFieldData& destination)
    {
        ...
        for (std::size_t c = 0; c < N; ++c)
        {
            ...
            core::FieldBox<Grid_t> dst{dst_grid, gridLayout,
                                       dual_dir_minus_1(as_unsigned_phare_box<dimension>(AMRToLocal(
                                                            intersectBox, destinationBox)),
                                                        dst_grid.physicalQuantity())};
            core::FieldBox<Grid_t const> const src{
                src_grid, source.gridLayout,
                dual_dir_minus_1(
        ...
        if (transformation.getRotation() == SAMRAI::hier::Transformation::NO_ROTATE)
        {
            ...
            if (transformation.getBeginBlock() == transformation.getEndBlock())
            {
                for (auto const& box : boxList)
                {
                    ...
                    if (!intersectionBox.empty())
                        copy_(intersectionBox, transformedSource, destinationBox, source, dst);
                }
            }
        }
        else
