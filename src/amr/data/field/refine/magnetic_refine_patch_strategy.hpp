        ...
        if constexpr (core::is_field_tile_set_v<Field_t>)
        {
            for (std::size_t ti = 0; ti < bx().size(); ++ti)
            {
                ...
                auto const tile_fine_field_boxes = std::array{
                    shrink(bx_tile.ghost_box(), 1),
                    shrink(by_tile.ghost_box(), 1),
                    shrink(bz_tile.ghost_box(), 1),
                };

                fix(bx_tile(), by_tile(), bz_tile(), tile_layout, tile_fine_field_boxes);
            }

            // domain cells for all tiles are now filled; propagate into inter-tile ghost regions
            bx.sync_inner_ghosts();
            by.sync_inner_ghosts();
            bz.sync_inner_ghosts();
        }
        else
        {
