...
class MagneticFieldRegrider // probably bad rebase, tbd
    : public AnyFieldRefiner<MagneticFieldRegrider<dimension>, dimension>
{
public:
    using Super = AnyFieldRefiner<MagneticFieldRegrider<dimension>, dimension>;
    using Super::Super;
    ...
    void refine(FieldT const& coarseField, FieldT& fineField, core::Point<int, dimension> fineIndex)
    {
        ...
        if constexpr (dimension == 1)
        {
            ...
            if (centerings_[0] == core::QtyCentering::primal)
            {
                if (auto& fineVal = fineField(locFineIdx);
                    fineIndex[0] % 2 == 0 && std::isnan(fineVal))
                {
        ...
        else if constexpr (dimension == 2)
        {
                ...
                and centerings_[dirY] == core::QtyCentering::dual)
            {
                // Bx
                if (auto& fineVal = fineField(locFineIdx);
                    fineIndex[dirX] % 2 == 0 && std::isnan(fineVal))
                {
                     ...
                     and centerings_[dirY] == core::QtyCentering::primal)
            {
                // By
                if (auto& fineVal = fineField(locFineIdx);
                    fineIndex[dirY] % 2 == 0 && std::isnan(fineVal))
                {
        ...
        else if constexpr (dimension == 3)
        {
                     ...
                     and centerings_[dirZ] == core::QtyCentering::primal)
            {
                // Bz
                if (auto& fineVal = fineField(locFineIdx);
                    fineIndex[dirZ] % 2 == 0 && std::isnan(fineVal))
                {
                    // we're on a coarse X face
                    // take the coarse face value
                    fineVal = coarseField(locCoarseIdx);
