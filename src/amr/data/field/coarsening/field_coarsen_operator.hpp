                     ...
                     SAMRAI::hier::IntVector const& ratio) const override
        {
            ...
            if constexpr (core::is_field_tile_set_v<FieldT>)
            {
                ...
                for (auto const& src_tile : sourceField())
                {
                    ...
                    if (auto const src_overlap = src_box * refine_box(phare_box))
                    {
                        for (auto& dst_tile : destinationField())
                        {
                            auto const dst_box = dst_tile.ghost_box();
                            if (auto const dst_overlap = dst_box * phare_box)
                            {
                                FieldCoarsenerPolicy coarsener{destinationLayout.centering(qty),
                                                               samrai_box_from(src_box),
                                                               destinationBox, ratio};
                                for (auto const& bix : *dst_overlap)
