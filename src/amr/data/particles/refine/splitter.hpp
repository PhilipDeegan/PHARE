...
private:
...

    {
        ...
        core::apply(patterns, [&](auto const& pattern) {
            ...
            {
                // SoA has no begin()/end() (no single Particle_t& to hand back) -- write
                // through the per-index view instead, same as python3/particles.hpp's
                // splitPyArrayParticles does for its input side.
                if constexpr (Particles::layout_mode == core::LayoutMode::SoA)
                {
                    core::SoAParticleView fineParticle{particles, idx++};
