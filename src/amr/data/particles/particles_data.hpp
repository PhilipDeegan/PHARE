...
namespace amr
{
        ...
        void putToRestart(std::shared_ptr<SAMRAI::tbox::Database> const& restart_db) const override
        {
            ...
            auto putParticles = [&](std::string const& name, auto& particles) {
                ...
                core::apply(soa.as_tuple(), [&](auto const& arg) {
                    ...
                    auto& vec = [](auto const& v) -> auto const& {
    ...
    private:
        ...
        // template<typename OverlapBox>
        // auto iterator_for_overlap(OverlapBox const& overlap) const
        // {
        //     auto local_lap = overlap * this->getBox();
        //     auto local_box = phare_box_from<dim>(local_lap);

        //     PHARE_LOG_LINE_STR("iterator_for_overlap");

        //     PHARE_LOG_LINE_STR(phare_box_from<dim>(local_lap));
        //     PHARE_LOG_LINE_STR(phare_box_from<dim>(overlap));

        //     PHARE_LOG_LINE_STR(domain_partition_iterators.size());

        //     for (auto const& box_range : domain_partition_iterators)
        //     {
        //         PHARE_LOG_LINE_STR(box_range.box());
        //         if (box_range.box() == local_box)
        //             return box_range;
        //     }
