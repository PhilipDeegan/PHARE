...
namespace amr
{
    ...
    private:
        ...
        void registerGhostComms_(std::unique_ptr<MHDMessengerInfo> const& info)
        {
            ...
            /// @todo suppress elecGhostRefiners ?
            elecGhostsRefiners_.addStaticRefiners(info->ghostElectric, EfieldRefineOp_,
                                                  info->ghostElectric,
                                                  nonOverwriteInteriorTFfillPattern);

            registerGhostRefinePatchStrategies_(currentPatchStratPerGhostRefiner_,
                                                info->ghostCurrent);
            ...
            for (size_t i = 0; i < info->ghostDensity.size(); ++i)
                rhoGhostsRefiners_.addTimeRefiner(
                    info->ghostDensity[i], info->modelDensity, rhoOld_.name(), mhdFieldRefineOp_,
                    fieldTimeOp_, info->ghostDensity[i], nonOverwriteFieldFillPattern,
            ...
            for (size_t i = 0; i < info->ghostMomentum.size(); ++i)
                momentumGhostsRefiners_.addTimeRefiner(
                    ...
                    nonOverwriteInteriorTFfillPattern, momentumPatchStratPerGhostRefiner_[i]);
            ...
            registerGhostRefinePatchStrategies_(magFluxesXPatchStratPerGhostRefiner_,
                                                info->ghostMagneticFluxesX);
            for (size_t i = 0; i < info->ghostMagneticFluxesX.size(); ++i)
                magFluxesXGhostRefiners_.addStaticRefiner(
                    info->ghostMagneticFluxesX[i], mhdVecFluxRefineOp_,
                    info->ghostMagneticFluxesX[i], nonOverwriteInteriorTFfillPattern,
                    magFluxesXPatchStratPerGhostRefiner_[i]);

            registerGhostRefinePatchStrategies_(magFluxesYPatchStratPerGhostRefiner_,
                                                info->ghostMagneticFluxesY);
            for (size_t i = 0; i < info->ghostMagneticFluxesY.size(); ++i)
                magFluxesYGhostRefiners_.addStaticRefiner(
                    info->ghostMagneticFluxesY[i], mhdVecFluxRefineOp_,
                    info->ghostMagneticFluxesY[i], nonOverwriteInteriorTFfillPattern,
                    magFluxesYPatchStratPerGhostRefiner_[i]);

            registerGhostRefinePatchStrategies_(magFluxesZPatchStratPerGhostRefiner_,
                                                info->ghostMagneticFluxesZ);
            for (size_t i = 0; i < info->ghostMagneticFluxesZ.size(); ++i)
                magFluxesZGhostRefiners_.addStaticRefiner(
                    info->ghostMagneticFluxesZ[i], mhdVecFluxRefineOp_,
                    info->ghostMagneticFluxesZ[i], nonOverwriteInteriorTFfillPattern,
                    magFluxesZPatchStratPerGhostRefiner_[i]);
        ...
        VectorFieldRefinePatchStrategyList magFluxesXPatchStratPerGhostRefiner_;
        VectorFieldRefinePatchStrategyList magFluxesYPatchStratPerGhostRefiner_;
        VectorFieldRefinePatchStrategyList magFluxesZPatchStratPerGhostRefiner_;
    };
