...
FFF=("${BUILD_DIR}")
CMAKE_CONFIG="-DdevMode=OFF -Dasan=OFF -Dbench=OFF -DwithCaliper=OFF -DtestMPI=OFF -DtestDuringBuild=OFF"
CMAKE_CONFIG="${CMAKE_CONFIG} -Dphare_configurator=ON " # -DHDF5_ROOT=/usr/local/HDF_Group/HDF5/1.15.0" #  " #-DHDF5_ROOT=/opt/mpi/rocm_hdf5"
CMAKE_CONFIG="${CMAKE_CONFIG} -DwithPhlop=ON " # -DPHARE_EXEC_LEVEL_MAX=99 "
...
CMAKE_CONFIG="${CMAKE_CONFIG} -DCMAKE_BUILD_TYPE=Debug" # Or RelWithDebInfo Release
...
time (
  ...
  (
    ...
    ninja -C ${BUILD_DIR} -v -j${THREADS} $@ # cpp cpp_etc dictator
    # ctest -j${THREADS}
  )
  date
