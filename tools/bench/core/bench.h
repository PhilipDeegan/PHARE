...
struct HybridPatch
{
    ...
    using IonPopulation_t = typename PHARE_TYPES::IonPopulation_t;
    ...
    struct State : public Layout, public Particles, public Vars<0>
    {
        ...
        using ParticleArray_t           = typename HybridPatch<GridLayout>::ParticleArray_t;
        using IonPopulation_t           = typename HybridPatch<GridLayout>::IonPopulation_t;
        using Vars_t                    = Vars<0>;
...
struct ParticleRange : public Range<Iterator>
{
    ...
    ParticleRange(Range<Iterator>&& range, std::shared_ptr<StateView> view_)
        : Super{std::forward<Range<Iterator>>(range)}
        , view{view_}
    {
    }
    ParticleRange(Range<Iterator> const& range, std::shared_ptr<StateView> view_)
        : Super{range}
        , view{view_}
    {
    }
