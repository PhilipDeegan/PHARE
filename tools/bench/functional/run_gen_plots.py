...
python3 -uO tools/bench/functional/run_gen_plots.py
...
from pyphare.core.phare_utilities import NO_GUI
...
def parse_cli_args():
    ...
    default_cli_args = DefaultCLIArgs()
    ...
    parser.add_argument(
        "--use_ninja",
        default=default_cli_args.use_ninja,
        type=strtobool,
        help="default: used if found",
    )
    parser.add_argument(
        "--use_ccache",
        default=default_cli_args.use_ccache,
        type=strtobool,
        help="default: used if found",
    )
    parser.add_argument(
        "--use_found_build_for_top",
        default=default_cli_args.use_found_build_for_top,
        type=strtobool,
...
def plot_scatter(test_case, cli_args, file_results, getter, type, git_hash):
    ...
    axis_legs = [
        f"{str(i).zfill(str_len)}_{k}" for i, k in enumerate(file_results.keys())
    ]
    ...
    for i, k in enumerate(file_results):
        ...
        data = np.array([getter(v[repeat]) for repeat in range(cli_args.repeat_stat)])
...
def verify_cli_args(cli_args):
    ...
    assert all([tool in available_tools for tool in cli_args.tools])
    return cli_args
...
def generate_test_cases(test_cases):
    ...
    def generate(test_case):
        module = importlib.import_module(
            f"tools.bench.functional.{test_cases_dir}.{test_case}"
        )
        module.generate_all()
    ...
    for test_case in test_cases:
        if test_case not in available_test_cases:
            raise RuntimeError(
                f"test_case ({test_case}) does not exist in subdirectory {test_cases_dir}"
            )
        generate(test_case)
...
def run_caliper(test_cases, cli_args, git_hash, mode=0):
    ...
    for test_case in test_cases:
        ...
        for file_name in scan_dir(bin_dir, files_only=True):
            ...
            module = importlib.import_module(py_file_module)
            with open(
                os.path.join(str(bindata_dir), "params.dill"), "wb"
            ) as write_file:
