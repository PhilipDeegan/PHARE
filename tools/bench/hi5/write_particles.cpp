...
void do_bench(benchmark::State& state)
{
    ...
    auto createDataSet_ = [&](auto& hi5, auto const& path, auto const& particles,
                               auto const& value) {
        using ValueType  = std::decay_t<decltype(value)>;
        auto const shape = hdf5::ParticleWriter::size_for<dim>(value, particles);
    ...
    while (state.KeepRunning())
    {
        ...
        core::apply(Packer::empty(), [&](auto const& arg) {
            datasets.emplace_back(
                createDataSet_(hi5, path + keys[part_idx], particles, arg));
            part_idx++;
        });
