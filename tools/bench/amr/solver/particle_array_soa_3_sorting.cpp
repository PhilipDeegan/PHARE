...
#include "core/data/particles/particle_array_soa_3.h"
...
static std::vector<std::int64_t> TOT{10 /*, 100*/};
static std::vector<std::int64_t> CELLS{10 /*0, 200, 300, 400, 500*/};
...
struct SortFixture : public benchmark::Fixture
{
    ...
    using ParticleArray_t = core::ParticleArray_SOA_3<dim>;
...
public:
    ...
    void operator()(::benchmark::State& state)
    {
        for (auto& pop : patch->ions)
            for (std::size_t i = 0; i < pop.domainParticles().size(); ++i)
                std::cout << __FILE__ << " " << __LINE__ << " "
                          << to_string(pop.domainParticles().iCell[i]) << std::endl;
        ...
        for (auto& pop : patch->ions)
        {
            ...
            for (std::size_t i = 1; i < pop.domainParticles().size(); ++i)
            {
                ...
                abort_if(prev->iCell == next.iCell or prev->iCell > next.iCell);
